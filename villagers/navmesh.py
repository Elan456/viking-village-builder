...
class NavMesh:
    ...
    def can_see(self, point1, point2):
        ...
        assert self.building_quadtree is not None, "Building quadtree not generated"
        ...
        buildings_to_check = [e.obj for e in buildings_to_check]
    ...
    def generate_building_quadtree(self):
        ...
        for building in self.village.buildings + [c.building for c in self.village.builder_manager.construction_queue]:
            ...
            # Insert the whole building as a single rectangle
            # (min.x, min.y, max.x, max.y)
            self.building_quadtree.insert(
                geom=(building.x - GRID_SIZE, building.y - GRID_SIZE,
                      building.x + building.rect.width + GRID_SIZE, building.y + building.rect.height + GRID_SIZE),
                obj=building
            )
    ...
    def generate_navmesh(self):
        ...
        for x in range(int(-defines.WORLD_WIDTH * defines.GRID_SIZE * .25),
            ...
            for y in range(int(-defines.WORLD_HEIGHT * defines.GRID_SIZE * .25),
                ...
                self.nodes_quadtree.insert((new_node.x, new_node.y), obj=new_node)

        # Add the wall nodes
        for node in self.village.wall.outer_corner_nodes:
            n = copy.deepcopy(node)
            self.nodes.append(n)
            self.nodes_quadtree.insert((n.x, n.y), obj=n)
        wall_node = copy.deepcopy(self.village.wall.hole_node)
        self.nodes.append(wall_node)
        self.nodes_quadtree.insert((wall_node.x, wall_node.y), obj=wall_node)
        ...
        for node in nodes_to_remove:
            ...
            self.nodes_quadtree.delete_by_object(node)
    ...
    def find_path_a_star(self, start_pnt, end_pnt):
        ...
        if start is None:
            raise ValueError("No start node found in navmesh")

        open_list = [start]
        closed_list = []
        came_from = {start: None}  # Dictionary to store the parent of each node
        assert self.nodes is not None, "Navmesh nodes not generated"
    ...
    def draw(self, surface):
        ...
        if self.building_quadtree is None:
            return

        # all_bbox = self.nodes_quadtree.get_all_bbox()
        all_bbox = self.building_quadtree.get_all_node_boundaries()
        ...
        for point in [v.geom for v in self.building_quadtree.get_all_items()]:
